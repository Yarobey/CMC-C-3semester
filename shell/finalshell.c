...
char conditions(char** list, char fl){
    char exit_st = 0;
    char condition = 0;
    char** cur;
    int k = 0;
...
char background(char** list){
    ...
    if((pid = fork()) == 0){    /* cын */
        if(fork() == 0){        /* внук */
            ...
            conditions(list, 1);    /* выполнение внука */
            sleep (20);
            exit(0);
        }
        exit(0);                        /* конец сына */
    ...
    int st;
    waitpid(pid, &st, 0);
    if(WIFEXITED(st) == 0)      /* завершился плохо */
        return 1;
    else
...
/* отправляем команды либо в фоновый режим, 
либо на обычное выполнение, ожидаем пока завершится выполнение сына,
    ...
    if((pid = fork()) == 0){   /* сын */
        ...
        while(1){
            ...
            if((list[i] != NULL) && (strcmp(list[i], "&") == 0))   /* переход в фоновый режим */
                exit_st |= background(current_list);       
            else                                                  /* переходим в состояние выполнения команды */
                exit_st |= conditions(current_list, 0);
            if(list[i++] != NULL)
                if(list[i] == NULL){
                    free(current_list);
                    break;
...
int main(){
    ...
    while(1){
        ...
        if(user_str != NULL){
            ...
            exit_st = interpret(user_str);           /* отправляем команду на выполнение */
            ...
            delete(user_str);         /* освобождаем память перед завершнием команды  */
        }
    }
    return 0;
}
